...
| IRF-SYS-070 | **P2** | **Prompt corpus maintenance** — periodic re-extraction via `organvm prompts narrate` + seed classification via `extract-seeds.py`. Gemini parser fixed S-prompt-archaeology-2 (was zero-extracting 56 sessions due to field name mismatch). Corpus at `praxis-perpetua/prompt-corpus/`. Current: 5,883 sequenced prompts, 853 seeds across 35 agents, 133-day span. Re-run when new sessions accumulate significantly. | Agent | S-prompt-archaeology-2 | None |
...
| IRF-SYS-073 | **P1** | **VACUUM: Supplementary source parsers missing from engine.** `organvm prompts narrate` covers Claude/Gemini/Codex sessions only. 4 source types require standalone scripts outside the engine: ChatGPT MD exports (213 files), Conductor traces (~30+ JSONL), SpecStory history (19 files), Claude /export TXT (5 files). Currently handled by `praxis-perpetua/prompt-corpus/ingest-supplementary.py` but not integrated into the engine pipeline. Either lift parsers into `prompts/extractor.py` or formalize the two-stage pipeline (engine + supplementary) as the canonical architecture. | Agent | S-prompt-archaeology-2 vacuum | None |
