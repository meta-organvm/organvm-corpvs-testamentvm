...
| IRF-CCE-015 | P1 | **Ratify OM-MEM-001 in omega scorecard** — proposed criterion text: "Memory infrastructure demonstrates closed-loop autopoiesis." Evidence exists (Claude corpus all 8 gates pass). Run `organvm omega amend --criterion OM-MEM-001` or manually update omega status JSON. Latest omega file (2026-03-19) is 0 bytes | Agent | S33 close-out N/A audit | Omega status file empty (needs regeneration first) |
| IRF-CCE-016 | P2 | **Enhance Claude adapter to parity with ChatGPT** — ChatGPT adapter now extracts code/execution/multimodal content, builds audit trails, detects near-duplicates. Claude adapter (`import_claude_export_corpus.py`) lacks all three. Cherry-pick the same patterns, then re-import Claude export corpus | Agent | S33 roadmap M3/E2 | None |
| IRF-CCE-017 | P2 | **Test remaining 10 untested modules** — 3,277 lines across federated_canon (1,056), cli (822), claude_local_session (324), source_lifecycle (309), provider_catalog (204), import_claude_local_session_corpus (189), provider_exports (142), provider_discovery (110), governance_policy (106), paths (15). Priority: federated_canon → answering (deepen from 3 tests) → source_lifecycle → cli | Agent | S33 roadmap M1 | None |
| IRF-CCE-018 | P2 | **Add semantic similarity triage policy** — remaining 833 entity-alias items need Levenshtein/token-overlap comparison on canonical_label (not just ID). Threshold ~0.85 could resolve ~400 more items. Also: title token jaccard for 344 family-merge items | Agent | S33 roadmap M2 | None |
...
| IRF-PRT-011 | P1 | **Shibui Phase 2B: Migrate remaining 19 project pages** — extract script produced 215 content units in 21 YAML sidecar files. 19 units have AI-generated entry text, 196 retain [DRAFT] markers (gemini CLI intermittent). Next: re-run `shibui:distill` when API stable, human-review generated text, wrap project page top sections in ShibuiContent. Pattern established by ai-conductor migration. | Agent | S35 Shibui session | Gemini API availability for remaining drafts |
| IRF-PRT-012 | P2 | **Shibui Phase 3: Per-page depth floors for essays** — `data-shibui-page-floor` attribute on `<main>` locks pages to minimum depth. Required before logos (essay) pages can be migrated. Spec designed but not implemented. | Agent | S35 Shibui spec | IRF-PRT-011 |
| IRF-PRT-013 | P2 | **Shibui: Run shibui:annotate pipeline** — annotation script ready but not yet executed. Generates term definitions for ShibuiTerm components across all 21 project YAML files. | Agent | S35 Shibui session | Gemini API availability |
...
| IRF-APP-014 | P1 | Fix reconcile_outreach.py parser — contact name misattribution in LinkedIn DM history ingestion. Parser loses track of contact boundaries, attributes DMs to wrong people. Known bug logged 2026-03-24, tool committed but not functional for batch reconciliation | Agent | Session S34 (outreach data gap) | None |
| IRF-APP-015 | P0 | Creative Capital 2027 application — opens 2026-04-02 (9 days). Qualified entry exists (`creative-capital-2027`). Highest-leverage single action across all three pillars. Must advance to drafting → staged → submitted before window closes | Human+Agent | Session S34 (Pillar 2 audit) | Window not yet open |
| IRF-APP-016 | P1 | Advance 4 remaining qualified Pillar 2 entries: LACMA Art+Tech, Whiting Nonfiction, Pioneer Works, Eyebeam Plurality. All qualified, all sitting. Target: 1 grant/residency submission per week | Agent | Session S34 (Pillar 2 audit) | None |
...
| IRF-APP-019 | P1 | Separate scoring rubrics per pillar — jobs, grants, consulting each need different dimensions. Jobs: remote_flexibility, studio_alignment. Grants: cycle_urgency, prestige_multiplier, narrative_fit. Consulting: deal_size, recurring_potential, client_fit | Agent | Session S34 (three-pillar strategy) | IRF-APP-018 |
| IRF-APP-020 | P1 | DMs for 7 connected-but-not-DM'd submitted entries — Anthropic, Coinbase, Cursor, Doppler, Harvey AI, LangChain, Render, Snorkel, Tailscale, Webflow. Protocol composer ready, just needs execution | Human | Session S34 (warm-path audit) | None |
...
| IRF-APP-024 | P1 | Outreach data persistence gap — 30 DMs sent, system logged 19 before manual backfill. Root cause: 3 files need updating per DM (outreach-log, contacts, network), no single ingestion point. Need `log-dm` single command AND fix reconcile parser (IRF-APP-014) | Agent | Session S34 (data gap) | None |
...
| DONE-221 | **Institutional strategy research** — exhaustive 4-track parallel research: 12 hybrid org structures analyzed (fiscal sponsorship + LLC scores 33/35), 40+ funding sources surveyed, 13 fiscal sponsors compared (Aspiration Tech recommended, Model C, 3-15%), Candid FSP pilot pathway mapped. 9 research docs (3,014 lines, ~164KB) committed to corpus. | S37 | 2026-03-24 |
| DONE-222 | **Grant application research** — NLnet NGI0 Commons (deadline Apr 1, full form fields/criteria extracted), Creative Capital 2027 (deadline Apr 2, full handbook analyzed), Aspiration Tech (application process mapped), LLC formation (1-day path documented), GitHub Sponsors (setup guide). Shuttleworth Fellowship confirmed CLOSED (July 2022 — grant aggregator misinformation). | S37 | 2026-03-24 |
| DONE-223 | **Institutional strategy master plan** — phased roadmap: Phase 1 (fiscal sponsor + LLC, weeks), Phase 2 (grant pipeline, months 1-6), Phase 3 (PBC conversion, year 1-3), Phase 4 (full Mozilla model, year 3+). Organ-to-entity mapping defined. Open-core IP model designed. | S37 | 2026-03-24 |
| DONE-224 | **14 new IRF items catalogued (IRF-INST-001 through IRF-INST-014)** — institutional strategy domain created. 2 P0 items (NLnet Apr 1, Creative Capital Apr 2), 5 P1 items (fiscal sponsor, LLC, IP policy, GitHub Sponsors, Candid email), 5 P2 items (Candid seal, donor platforms, Sovereign Tech, ZKM, Sloan), 2 P3 items (university affiliate, Shuttleworth replacements). | S37 | 2026-03-24 |
...
| ID | Priority | Action | Owner | Source | Blocker |
|----|----------|--------|-------|--------|---------|
| IRF-INST-001 | **P0** | **Apply to NLnet NGI0 Commons Fund** — EUR 5K-50K for governance tooling as open-source infrastructure. Web form at nlnet.nl/propose/. Frame: generalizable promotion state machines, registry management, seed.yaml contracts for any large OSS ecosystem. Deadline: **April 1, 2026 12:00 CEST**. Research complete at `docs/applications/2026-03-24-nlnet-application-research.md` | Human | S37 institutional strategy | Need project abstract, budget breakdown |
| IRF-INST-002 | **P0** | **Apply to Creative Capital Award 2027** — $15K-$50K unrestricted. Apply under Visual Arts + Technology sub-discipline. Need: 500-word project description, 4x 150-word questions, 1 completed work sample, 200-word bio, resume PDF. Deadline: **April 2, 2026 3:00 PM ET**. Research complete at `docs/applications/2026-03-24-creative-capital-application-research.md` | Human | S37 institutional strategy | Need work sample (completed past artwork) |
...
| IRF-INST-009 | **P2** | **Apply to Sovereign Tech Fellowship** — Deadline April 6, 2026. OSS maintenance/docs work. Needs research on exact application process. | Human | S37 institutional strategy | Need research |
| IRF-INST-010 | **P2** | **Apply to ZKM Rauschenberg Residency** — Deadline April 12, 2026. Media art research. Needs research on exact application process. | Human | S37 institutional strategy | Need research |
