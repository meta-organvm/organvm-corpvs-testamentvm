...
| IRF-APP-015 | P0 | Creative Capital 2027 application — opens 2026-04-02 (8 days). **ADVANCED to drafting 2026-03-25.** Materials exist: resume (batch-03, PDF), cover letter (alchemized), profile JSON (3-length statements/bios/samples). Remaining: fill portal when it opens April 2. | Human+Agent | Session S34→S35 | Window not yet open |
...
| IRF-APP-026 | P2 | Content engine pitch deck: custom Netlify domain — current URL is auto-generated (`euphonious-semolina-ef6428.netlify.app`). Needs branded domain (e.g., `cronus-metabolus.netlify.app` or custom). Not client-shareable in current state | Agent | Session S37 (pitch deck vacuum) | None |
...
| IRF-APP-030 | P3 | Content engine: `consumers: []` vacuum in seed.yaml — no consumers declared despite pitch decks targeting three buyer segments (clients, agencies, DTC brands). Should declare consumption edges once MVP architecture solidifies | Agent | Session S37 (seed.yaml vacuum) | IRF-APP-004 (MVP build) |
...
| IRF-APP-031 | **P1** | **Send pricing to Maddie** — $1K deposit + $200/month retainer + case study rights. Draft exists in memory. Must be sent via text (not phone). Blocking dependency for studio revenue. Window is NOW — client is in "cry scream throw up so happy" phase. | Human | S-elevate (pricing) | Human action |
| IRF-APP-032 | P1 | sovereign-systems: register in registry-v2.json — 30th ORGAN-III repo. type: client-project, tier: standard, promotion: LOCAL, status: ACTIVE. Currently absent from registry | Agent | S-elevate (vacuum) | None |
| IRF-APP-033 | P1 | sovereign-systems: connect custom domains — elevatealign.com (GoDaddy DNS), stopdrinkingacid.com (LeadConnector DNS). Requires client action (DNS change). Domain setup guide written at docs/domain-setup.md | Human | S-elevate (deploy) | Maddie DNS access |
