...
| IRF-MON-004 | P1 | **Density delta rendering bug** — `Δ24h: n/a | Δ7d: n/a` in CLAUDE.md. **NOT a missing-data problem** — 773 AMMOI snapshots exist in `~/.organvm/pulse/ammoi-history.jsonl` (2.9MB, March 14–present). Root cause: (a) Python truthiness bug at `contextmd/generator.py:656-657` — `if d24h else "n/a"` treats `0.0` (stable density, correct answer) as falsy → renders "n/a". Fix: `if d24h is not None`. (b) Semantic ambiguity: `_compute_temporal_deltas()` returns `0.0` for BOTH "no historical data" and "genuinely no change" — change to return `None` for no data. Plan: `.claude/plans/2026-03-25-vacuum-audit-nda-remediation.md` §V1 | Agent | S35 N/A deep audit | None |
